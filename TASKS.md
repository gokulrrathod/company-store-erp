...
- [x] **Re-recorded all demo videos** against the current UI (post sidebar/header/notifications/grid-density overhaul) — `demo-videos/2026-08-21-latest/`: one combined narrated walkthrough (`00-full-demo-all-8-modules.webm`) plus 13 individual per-workflow videos (one per regression test, `01`–`13`), giving separate close-up footage of each module instead of only the single combined video. Old stale `VMG-ERP-Full-Demo-2026-08-21.webm` removed.
