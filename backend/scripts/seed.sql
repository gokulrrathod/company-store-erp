-- Deployable copy of db/seed.sql (repo root), kept in sync manually.
-- Docker's build context for the backend service is rooted at backend/,
-- so it can't COPY files from the sibling db/ folder — this copy exists
-- so reset-test-data.js has something to read once deployed. If you
-- change db/seed.sql, mirror the change here too.
