...
router.post('/', requireRole(ROLES.STORE_EXECUTIVE, ROLES.STORE_MANAGER, ROLES.QUALITY, ROLES.ADMIN), validate(safetySchema), async (req, res) => {
...
router.patch('/:id/close', requireRole(ROLES.STORE_MANAGER, ROLES.ADMIN), async (req, res) => {
  const { rows } = await pool.query(
